function [ root_correct ] = findRoot2(x0, tol)
...
root_correct = newtonRoot2(f,fDeriv);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ root_correct] = newtonRoot2(f,fDeriv)
...
hold all
plot(3,f(3), '-bs', 'MarkerEdgeColor', 'blue');
plot(2.2,f(2.2), '-mo', 'MarkerEdgeColor', 'red');
grid on;
title('Roots of f(x)=3*x^4 -5*x^3 - 12*x^2 Using Newtons Method: Nested Function')
xlabel('f(x)');
ylabel('x');
legend('f (x)', 'f(x)=0', 'f(x0)', 'location','north');
saveas(gcf,'findRoot2.pdf')
end