...
r=37;
plotsOn='no';
[prctBias, prctRMSE, ntrialVec]=p01Masuyama(ntrials,csvfile,r,plotsOn);
tol1=1.5e-1;
tol2=1.5e-1;
assert( max(abs(prctBias(end-5:end)))< tol1 && max(abs(prctRMSE(end-5:end)-50.5))< tol2,  ...
    ...
    'prctRMSE = [' sprintf(' %5.3f ',prctRMSE(end-5:end)) ']\n' ...
        ...
        'Expected output prctRMSE within ' sprintf(' %5.3f ',tol2) ' of 50.5 \n'], ...
...
tol1=1e-1;
tol2=1e-2;
assert( max(abs(prctBias(end-5:end)))< tol1 && max(abs(prctRMSE(end-5:end)-50.28))< tol2,  ...
    [ '\nYour output (last 5 entries) \nprctBias = [' sprintf(' %5.3f ',prctBias(end-5:end)) ']\n' ...
    'prctRMSE = [' sprintf(' %5.3f ',prctRMSE(end-5:end)) ']\n' ...
        'Expected output prctBias within ' sprintf(' %5.3f ',tol1) ' of 0 \n' ...
        'Expected output prctRMSE within ' sprintf(' %5.3f ',tol2) ' of 50.28 \n'], ...
        prctBias(end-5:end),prctRMSE(end-5:end),tol1,tol2)
...
time_student=toc
tic;
[prctBias, prctRMSE, ntrialVec]=p01MasuyamaRef(ntrials,csvfile,r,plotsOn);
time_solution=toc
